    ...
    for q in AI_QUERIES:
        try:
            r = requests.get(HN_SEARCH, params={
                "query": q,
                "tags": "story",
                "numericFilters": "created_at_i>" + str(yesterday) + ",points>10",
                "hitsPerPage": 10,
            }, timeout=10)
    ...
    for topic in AI_TOPICS:
        try:
            r = requests.get(GH_SEARCH, headers=headers, params={
                "q": "topic:" + topic + " created:>" + week_ago,
                "sort": "stars",
                "order": "desc",
                "per_page": 5,
            }, timeout=10)
...
def main():
    ...
    hn = fetch_hn()
    with open(str(data_dir) + "/hn_stories.json", "w") as f:
        json.dump(hn, f, indent=2)

    gh = fetch_github()
    with open(str(data_dir) + "/github_trending.json", "w") as f:
